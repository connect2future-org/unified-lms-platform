...
api.interceptors.request.use((config) => {
  const token = getUserToken() || getTeamToken() || getAdminToken()
  if (token) {
...
api.interceptors.response.use(
  ...
  (error) => {
    ...
    const isSessionProbe =
      ...
      requestUrl.includes('/auth/team/me') ||
      requestUrl.includes('/auth/platform-admin/me')

    if (status === 401 && typeof window !== 'undefined') {
      // Only force logout when session identity checks fail.
      // Other endpoint-level 401s should surface in-page instead of destroying auth state.
      if (isSessionProbe) {
        clearUserToken()
        clearTeamSession()
        clearAdminToken()
        if (window.location.pathname.startsWith('/team')) {
          window.location.assign('/team/login')
        } else {
...
export const loginAdmin = async (payload) => {
  ...
  if (data?.token) {
    if (data?.user?.role === 'admin' && data?.user?.authType === 'platform-admin') {
...
export const getUserUnifiedAuthMigrationSummary = async () => {
  const { data } = await api.get('/auth/admin/migration/users-unified-auth')
  return data
}

export const runUserUnifiedAuthMigration = async () => {
  const { data } = await api.post('/auth/admin/migration/users-unified-auth')
  return data
}
