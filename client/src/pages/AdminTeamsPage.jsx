...
const AdminTeamsPage = () => {
  ...
  const fetchMigrationSummary = async () => {
    try {
      const [registrationSummary, userSummary] = await Promise.all([
        getRegistrationMigrationSummary(),
        getUserUnifiedAuthMigrationSummary()
      ])
      setMigrationSummary(registrationSummary)
      setUserMigrationSummary(userSummary?.summary || null)
    } catch {
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      try {
        const [teamData, projectData, migrationData, lookupData, userSummaryData] = await Promise.all([
          getAdminTeams(),
          getProjects(),
          getRegistrationMigrationSummary(),
          getAdminRegistrationLookups(),
  ...
  const handleRunUserMigration = async () => {
    ...
    try {
      const response = await runUserUnifiedAuthMigration()
      ...
      await fetchMigrationSummary()
    } catch (requestError) {
      setError(requestError.response?.data?.message || 'User migration failed')
    } finally {
          ...
          <div className="mt-6 space-y-6">
            ...
            {activeFlow === FLOW_TABS.migration ? (
              <AdminMigrationSection
                ...
                onRunUserMigration={handleRunUserMigration}
